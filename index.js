...
const Clan = mongoose.model('Clan', new mongoose.Schema({
    ...
    applyChannelId: String, // Added applyChannelId
    members: [String],
    assistantIds: [String],
...
<div style="width:100%; min-height:100vh; display:flex; flex-direction:column; align-items:center; padding-top:40px;">
    <div style="font-size:40px; font-weight:bold; background: linear-gradient(45deg, #ff4d6d, #7b2ff7); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom:30px;">VORTEX</div>
        ...
        <table style="width:100%; color:white; border-collapse: collapse; text-align:right;">
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #222;">
                    ...
                    <td><a href="/delete-kick/${g.id}/${i}" style="color:var(--s); text-decoration:none;" onclick="return confirm('حذف؟')">حذف</a></td>
                </tr>`).join('')}
            </tbody>
        </table>
    </div>`;
...
app.get('/manage/:guildId/autoreply', checkAuth, async (req, res) => {
    ...
    <form method="POST" action="/save/${g.id}/autoreply">
        <div class="card">
            <h3>🤖 نظام الرد الآلي (15 رد)</h3>
            <p style="color: #aaa; font-size: 13px;">اكتب الكلمة والرد، واترك الحقول فارغة لحذف الرد.</p>
...
app.post('/save/:guildId/autoreply', checkAuth, async (req, res) => {
    try {
        ...
        res.status(500).send("خطأ في حفظ الردود الآلية");
    }
});
...
app.get('/manage/:guildId/clans', checkAuth, async (req, res) => {
    ...
    <div class="card">
        ...
        <div style="padding: 20px; background: rgba(255,255,255,0.03); border-radius: 15px; margin-bottom: 15px; border: 1px solid rgba(255,255,255,0.05);">
            ...
            <a href="/manage/${g.id}/clans/edit/${clan.clanIndex}" style="color: var(--p); text-decoration: none;">تعديل</a>
            <a href="/manage/${g.id}/clans/delete/${clan.clanIndex}" style="color: var(--s); text-decoration: none; margin-right: 15px;" onclick="return confirm('حذف الكلان؟')">حذف</a>
        </div>`).join('')}
    </div>`;
...
client.on('messageCreate', async (msg) => {
    ...
    if (msg.content.startsWith('!توب') || msg.content.startsWith('!top-streak')) {
        ...
        embed.setDescription(description).setFooter({ text: "VORTEX • المنافسة مشتعلة!" });
        msg.reply({ embeds: [embed] });
    }
    ...
    try {
        ...
        const controlRow = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
                ...
                .addOptions([
                    { label: 'استلام التكت', value: 'claim_ticket' },
                    { label: 'إغلاق التكت', value: 'close_ticket' },
                    { label: 'إضافة عضو', value: 'add_member' },
                    { label: 'إزالة عضو', value: 'remove_member' },
                    { label: 'استدعاء صاحب التكت', value: 'summon_member' }
                ])
        );
